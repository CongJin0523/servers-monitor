...
function updateCpuUsage(list) {
    ...
    singleSeries(option, 'CPU Usage(%)', data, ['#72c4fe', '#72d5fe', '#2b6fd733'])
    chart.setOption(option)
}
function updateMemoryUsage(list) {
    const chart = charts[1]
    let data = list.map(item => (item.memoryUsage * 1024).toFixed(1));
    const option = defaultOption('Memory(MB)', localTimeLine(list))
    singleSeries(option, 'Memory Usage(MB)', data, ['#6be3a3', '#bbfad4', '#A5FFD033'])
    chart.setOption(option);
}
function updateNetworkUsage(list) {
    ...
    const option = defaultOption('Network(KB/s)', localTimeLine(list))
    doubleSeries(option, ['Upload(KB/s)', 'Download(KB/s)'], data, [
...
function updateDiskUsage(list) {
    ...
    const option = defaultOption('Disk(MB/s)', localTimeLine(list))
    doubleSeries(option, ['Read(MB/s)', 'Write(MB/s)'], data, [
